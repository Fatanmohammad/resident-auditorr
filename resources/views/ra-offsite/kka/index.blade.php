...
<style>
    ...
    .kka-nav-tabs .nav-link:hover {
        color: var(--bs-blue-dark, #1e3a8a);
        background-color: #f8fafc;
    }
    .kka-nav-tabs .nav-link.active {
        color: #2563eb;
...
<div class="kka-container">
    ...
    <div class="grid grid-cols-4">
        ...
        <div class="stat-card">
            <div class="stat-icon yellow" style="background: #fef2f2; color: #dc2626;">
    ...
    <div class="kka-nav-tabs">
        @foreach($availableSheets as $key => $info)
            @php
                $routeParam = $info['route_param'] ?? str_replace('_', '-', $key);
            @endphp
    ...
    <div class="card">
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 9%;">TANGGAL DATA</th>
                            ...
                            <th style="width: 11%;">DATA SOURCE</th>
                            <th style="width: 8%; text-align: center;">RISK LEVEL</th>
                        ...
                        @forelse($items as $index => $item)
                            <tr>
                                ...
                                <td style="white-space: nowrap; font-size: 0.82rem;">
                                    {{ \Carbon\Carbon::parse($item->tanggal_data)->format('d/m/Y') }}
                                </td>
                                <td style="font-weight: 600; color: var(--bs-blue-dark); font-size: 0.83rem;">
                                    {{ $item->kode_unit }}
                                </td>
                                <td>
                                    <span class="badge badge-gray" style="font-family: monospace; font-size: 0.78rem; letter-spacing: 0.5px;">
                                        {{ $item->no_referensi ?? $item->case_id }}
                                    </span>
                                </td>
                                <td style="font-size: 0.82rem;">
                                    {{ $item->user_maker ?? 'System' }}
                                </td>
                                <td>
                                    <code style="font-size: 0.78rem;">{{ $item->kode_transaksi ?? '-' }}</code>
                                </td>
                                ...
                                <td>
                                    <span style="font-size: 0.83rem; color: #334155;" title="{{ $item->deskripsi_narasi }}">
                                        {{ Str::limit($item->deskripsi_narasi, 45) }}
                                    </span>
                                </td>
                                <td>
                                    <span class="badge badge-gray" style="font-size: 0.75rem;">
                                        {{ $item->data_source ?? 'cbs_transactions' }}
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colspan="11" style="padding: 3rem 1rem;">
                                    <div class="empty-state" style="text-align: center;">
                                        ...
                                        <p style="margin-top: 0.5rem; color: var(--text-muted); font-size: 0.9rem;">Belum ada data exception atau sampel risiko pada sheet <strong>{{ $sheetTitle }}</strong> ini.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        @if($items->hasPages())
            @php
                $activeRouteParam = $availableSheets[$currentSheet]['route_param'] ?? str_replace('_', '-', $currentSheet);
            @endphp
